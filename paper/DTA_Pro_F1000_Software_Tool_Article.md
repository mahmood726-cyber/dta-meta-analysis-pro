...
**WebR in-browser validation.** A distinctive feature of DTA Pro is its integrated WebR validation capability. Clicking the "Validate with WebR" button loads the R statistical environment directly in the browser via WebAssembly (WebR v0.4.4) and runs an independent bivariate GLMM using base R's `optim()` function on the current dataset — no R packages are required. This provides a lightweight, package-free cross-check that allows users and reviewers to verify pooled sensitivity, specificity, DOR, and AUC against an independent ML implementation without installing R or leaving the browser. Both DTA Pro and WebR use ML estimation; for REML-based validation against the R mada package, users can export and run the generated R script locally or via webr.r-wasm.org. The WebR output includes a 7-metric comparison table with pass/fail indicators against pre-specified tolerances.
